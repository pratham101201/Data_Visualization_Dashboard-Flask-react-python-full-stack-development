...
const Filters = ({ setMainData }) => {
    ...
    const getDataFromDB = async (year) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/data/year/${year}`); // Updated URL
    ...
    const handleReset = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/data/all"); // Updated URL
            setMainData(response.data.data);
        } catch (e) {
            console.log(e);
        }
